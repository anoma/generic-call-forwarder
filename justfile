...
# Recipes read `ALCHEMY_API_KEY` (deploys) from the environment; forge does not
# load this file itself. The file is absent in CI, where the values come from
# secrets instead, so loading it stays optional. `IS_TEST_DEPLOYMENT` is
# deliberately not kept here — see the release checklist, which exports it once
